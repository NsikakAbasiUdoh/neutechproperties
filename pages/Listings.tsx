...
import { PropertyContextType, PropertyType, PropertyCategory, PropertyStatus } from '../types';
...
const Listings: React.FC<ListingsProps> = ({ propertyContext }) => {
  const { properties, filterState, filterLGA, setFilterState, setFilterLGA, openRequestModal } = propertyContext;
  ...
  const filteredProperties = useMemo(() => {
    return properties.filter(p => {
      // Must be approved
      if (p.status !== PropertyStatus.APPROVED) return false;
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredProperties.length > 0 ? (
            filteredProperties.map(property => (
              <PropertyCard key={property.id} property={property} onRequest={openRequestModal} />
            ))
          ) : (
