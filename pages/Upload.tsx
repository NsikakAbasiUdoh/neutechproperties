...
const Upload: React.FC<UploadProps> = ({ propertyContext, onNavigate }) => {
  const { addProperty, publisherAccessCode, adminAccessCode, currentUser } =
    propertyContext;
  ...
  const handleGenerateDescription = async () => {
    if (!formData.title || !formData.state || !formData.featuresString) {
      setError(
        "Please fill in Title, State, and Features before generating a description.",
      );
      return;
    }
    setError("");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setSuccess(
          "Property uploaded successfully! It is now pending approval by an admin.",
        );
        ...
        <div className="min-h-screen flex items-center justify-center px-4 py-12">
          <div className="max-w-md w-full bg-white/95 backdrop-blur-md rounded-xl shadow-2xl p-8 text-center border border-white/20">
            ...
            <p className="text-gray-500 mt-2">
              Your agent account is currently waiting for admin approval. You
              cannot upload properties yet.
            </p>
          </div>
        </div>
      );
      ...
      <div className="min-h-screen flex items-center justify-center px-4 py-12">
        <div className="max-w-md w-full bg-white/95 backdrop-blur-md rounded-xl shadow-2xl overflow-hidden border border-white/20 p-8">
          <div className="text-center mb-8">
            ...
            <p className="text-gray-500 mt-2">
              Enter your Agent ID to list a property.
            </p>
          </div>
          ...
          <div className="bg-primary/90 px-8 py-6 flex justify-between items-center">
            <div>
              ...
              <p className="text-gray-400 mt-1">
                {currentUser
                  ? `Welcome Agent ${currentUser.name}`
                  : "Add a new land or house to the marketplace."}
              </p>
            </div>
            {currentUser ? (
            ...
            <div className="pt-5">
              ...
              >
                {isUploading ? (
                  ...
                  `Publish Property (${imageFiles.length} photos)`
                )}
              </button>
            </div>
          </form>
