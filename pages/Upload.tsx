...
import { uploadImage, uploadVideo } from '../services/supabaseClient';
import { Sparkles, Upload as UploadIcon, CheckCircle, AlertCircle, Loader2, XCircle, Lock, LogIn, Key, UserCheck, Plus, Image as ImageIcon, Trash2, Video } from 'lucide-react';
...
const Upload: React.FC<UploadProps> = ({ propertyContext, onNavigate }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (imageFiles.length === 0 && videoFiles.length === 0) missingFields.push("At least one Property Image or Video");
    ...
    try {
        ...
        if (videoFiles.length > 0) {
            const uploadPromises = videoFiles.map(file => uploadVideo(file, 'properties'));
            const uploadedUrls = await Promise.all(uploadPromises);
            ...
            if (validVideoUrls.length === 0) {
                throw new Error("Failed to upload videos. Please check your connection.");
            }
        }
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="bg-white/95 backdrop-blur-md rounded-xl shadow-2xl overflow-hidden border border-white/20">
          ...
          <form onSubmit={handleSubmit} className="p-8 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                >
                  ...
                  <option value={PropertyCategory.COMMERCIAL}>Vehicle</option>
                </select>
              </div>
            ...
            <div>
                <label className="block text-sm font-medium text-gray-700">Property Images (Optional if Video is provided)</label>
                <div className="mt-1">
            ...
            {/* Video Upload */}
            <div>
                <label className="block text-sm font-medium text-gray-700">Property Video (Optional)</label>
                <div className="mt-1">
                    <div className="flex items-center justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-md hover:bg-gray-50/50 transition-colors bg-white/50">
                        <div className="space-y-1 text-center w-full">
                            ...
                            <p className="text-xs text-gray-500">MP4, WebM, OGG (Max 10MB per file)</p>
                        </div>
                    </div>
                </div>
