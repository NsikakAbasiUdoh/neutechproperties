...
const DEFAULT_KEY =
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImF0aXNwaWFpc3pwdWR3bHBpcmt2Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3Njg3ODQ0ODEsImV4cCI6MjA4NDM2MDQ4MX0.mKqwtgzCyIyaDY4QmHv2ThxWmGhnu7YvArGERXQHPeQ";
...
const url =
  getEnvVar("VITE_SUPABASE_URL", "REACT_APP_SUPABASE_URL") || DEFAULT_URL;
const key =
...
export const logVisit = async () => {
  ...
  // Simple session check to avoid counting refreshes as new visits immediately
  if (sessionStorage.getItem("neutech_visited")) return;

  try {
    // 1. Get Location Data (using free IP API)
    // Note: In production, consider using a backend proxy or Edge Function for more reliability
    ...
    const { error } = await supabase.from("visits").insert([
      {
        ...
        // visited_at is auto-generated by DB default, but we can send it too
        visited_at: new Date().toISOString(),
      },
    ]);

    if (!error) {
      sessionStorage.setItem("neutech_visited", "true");
      console.log("Neutech Analytics: Visit logged from", data.city);
    } else {
      // If error (e.g. table doesn't exist), we silently fail so user experience isn't affected
      console.warn(
        "Neutech Analytics: Failed to log visit to DB",
        error.message,
      );
    }
  } catch (err) {
    console.warn(
      "Neutech Analytics: Logging skipped (Adblocker or Network Error)",
    );
  }
};
...
export const uploadVideo = async (
  ...
  try {
    if (!isConnected) {
      console.log(
        "Not connected to Supabase, converting video to base64 for demo.",
      );
      return await fileToBase64(file);
    }

    const ext = file.name.split(".").pop() || "mp4";
    const fileName = `${folder}/${Math.random().toString(36).substring(2)}_${Date.now()}.${ext}`;

    const { data, error } = await supabase.storage
      .from("images") // Reusing the same bucket for simplicity
...
export const uploadImage = async (
  ...
  try {
    ...
    if (!isConnected) {
      console.log(
        "Not connected to Supabase, converting compressed image to base64 for demo.",
      );
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.warn(
          `Public URL check failed (${response.status}), falling back to base64.`,
        );
        return await fileToBase64(compressedFile);
      }
    } catch (fetchError) {
      ...
      console.warn(
        "Public URL reachability check failed (CORS/Network), falling back to base64.",
        fetchError,
      );
      return await fileToBase64(compressedFile);
    }
