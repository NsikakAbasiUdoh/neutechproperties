...
import { MapPin, ArrowRight, Phone, Send, ImageOff, Video } from 'lucide-react';
...
const PropertyCard: React.FC<PropertyCardProps> = ({ property, onRequest }) => {
  ...
  const isVideo = (url: string | null | undefined) => {
    ...
    return cleanUrl.match(/\.(mp4|webm|ogg|mov|avi|mkv|m4v)$/i) || url.startsWith('data:video/');
  };

  const coverMedia = coverImage || (property.videoUrls && property.videoUrls.length > 0 ? property.videoUrls[0] : property.videoUrl);
        ...
        <div className="absolute bottom-4 right-4 flex flex-col gap-1 items-end">
          {((property.videoUrls && property.videoUrls.length > 0) || property.videoUrl) && (
            <div className="bg-black/70 backdrop-blur text-white text-xs font-bold px-3 py-1.5 rounded-lg shadow-sm flex items-center">
              <Video size={12} className="mr-1" /> {property.videoUrls && property.videoUrls.length > 1 ? `${property.videoUrls.length} Videos` : 'Video'}
            </div>
          )}
          {images.length > 1 && (
            <div className="bg-black/70 backdrop-blur text-white text-xs font-bold px-3 py-1.5 rounded-lg shadow-sm">
